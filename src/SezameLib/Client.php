...
class Client
{
    ...
    {
        return $this->_http->getLastRequest();
    }

    public function setMode($mode)
    {
        if ($mode == 'dev') //$this->_endpoint = 'https://hqfrontend-finprin.dev.finprin.com/';
        {
    ...
    {
        return new \SezameLib\Request\LinkDelete($this);
    }

    public function post($endpoint, $postdata = Array())
    {
        try {
            return $this->_http->post($this->_endpoint . $endpoint, Array('Content-Type' => 'application/json'),
    ...
    public function delete($endpoint, $postdata = Array())
    {
        try {
            return $this->_http->delete($this->_endpoint . $endpoint, Array('Content-Type' => 'application/json'),
                json_encode($postdata));
        } catch (\Buzz\Exception\RequestException $e) {
            $ce = new Exception\Connection($e->getMessage(), $e->getCode());
            $ce->setRequest($e->getRequest());
    ...
    {
        if ($response->isEmpty()) {
            throw new Exception\Response('Got empty response', $response->getStatusCode());
        }
        ...
        throw new Exception\Response($response->getReasonPhrase(), $response->getStatusCode());
    }
